...
//		AssertThat_TextByXpath(XpathCommon.WODEJIFEN_DANGQIANDENGJI, Rider_level);
		...
		ClickOnView("bg");
//		WaitForStringShowInWebView("查看明细 ");                
//		ClickOnTextInWebView("查看明细 "); 
...
//		ClickOnView("ok");
	 ...
	 */
		...
		public void test_0401_JiXiaoKaoHe_RiQi(){
			...
			if(!CheckStringIsShow("准时率","平均送达","完成单量","顾客差评","顾客差评")){
				System.err.println("没有获取到/logistics/rider/getknightkpidata接口的数据（日）");
				failAndsrceen("test_0401_JiXiaoKaoHe_未获取数据");
			}
...
//			WaitForLoadingBearDisappear();
	...
	public void test_0505_WoYaoTuCao_XiangQingMiaoShu() {
		...
		ClickOnText("投诉其他骑士");
		ClickOnView("et_desc");
		if(!CheckTextById("et_desc", "必填，请填写最多300字。如投诉请填写您要投诉对象的姓名，核实后我们会认真处理。")){
	...
	public void test_0602_WoDeYuE_XiuGai_TiXianPsw(){
		...
		Juge_TiXianMiMa_isRight("199109","199109","设置提现密码","修改提现密码---未限制密码不能为生日！！！");
		
		Juge_TiXianMiMa_isRight("910924","910924","设置提现密码","修改提现密码---未限制密码不能为生日！！！");
...
//			WaitForStringShowAndClick("提现");
	     ...
	     */
			@Test
			public void test_0600_WoDeYuE_TiXian_BianJiKuang(){
				Goto_UserCenter1("我的余额");
				String Before_BanlanceString =GetStringByid("tv_balance");  //存储当前可提现余额
				ClickOnText("提现");   //et_balance
...
//				System.err.println("*"+Before_BanlanceString+"*"+StrInFactNow+"*");
				if(!(StrInFactNow.startsWith(Before_BanlanceString))){
...
//	    	System.out.println(StrInFact);
	    	...
	    	ClickOnView("bg");
	    	WaitForStringShowInWebView(Strlist);
    	}
